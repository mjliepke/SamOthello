...
namespace SamOthellop
{
    class SamCode
    {
        ...
        public SamCode() //Default Constructor
        {
        ...
        private double forward(double[] inp, bool answer)
        {
            //outs[0] = inp;
            for (int i = 0; i < inp.Length; i++) { outs[0, i] = inp[i]};

            for (int l = 1; l < weights.Length; l++)
            {
                double[] inps;
                for (int pn = 0; pn < outs.GetLength(1); pn++) { appendItem(inps, outs[l - 1, pn])}

                for (int n = 0; n < weights.GetLength(1); n++)
                {
                    double sum = 0;
        ...
        public void back(int expected)
        {
            for (int i = 0; i < outs.GetLength(1); i++)
            {
                delta[outs.length - 1, i] = (expected[i] - outs[outs.GetLength(0) - 1, i]) * derivative(outs[outs.GetLength(0) - 1, i]);
            }
            for (int l = outs.GetLength(0) - 2; l > 0; l--)
            {
                for (int n = 0; n < outs.GetLength(1); n++)
                {
                    double sum = 0;
                    for (int fn = 0; fn < weights.GetLength(1); fn++)
                    {
            ...
            for (var wl = 1; wl < weights.GetLength(0); wl++)
            {
                for (var wn = 0; wn < weights[wl].length; wn++)
                {
                    for (var ww = 0; ww < weights.GetLength(2); ww++)
                    {
        ...
        private double round(double num)// Is this supossed to be a floor function? as in truncation?
        {
            return (Math.Round(10000 * num) / 10000);
        }

        private void train(double[] inp, double[] goal, bool display)
        {
            forward(inp); back(goal);

            double[] table = new double[outs.GetLength(1)];
            ...
            if (display)
            {
                console.log("expected: " + goal + ", output: " + table);
            }
        }

        private void initializeRandValues()
        {
            int[,] pieces;
            int[,] bplays;
            int[,] wplays;
            ...
            bool changecor = true;
            bool[,,] decisions; //Not quite sure the degree of this array at the moment
            int winner = 0;
        }

        private void setbox(int x, int y, int cor)
        {
            if (cor == 1)
            {
        ...
        private void resetboard()
        {
            for (int k = 0; k < 8; k++)
            {
                pieces[k] =[null, null, null, null, null, null, null, null, null];//invalid syntax for c#
        ...
        private void resetcalc(int cor)
        {
            doublt[] table;
            if (cor == 1)
            {
...
}//END OF CLASS SAMCODE


    var calcplays = function(cor){
                var table;
    nomoves = true;
                if (cor == 1) { table = wplays
} else { table = bplays}
                resetcalc(cor);
                for (var i = 0; i< 8; i++)
                {
...
var ax = x; var ay = y;
            ...
            var testtable =[];
            for (var aahh = 0; aahh< 8; aahh++)
            {
                testtable[aahh] =[]; for (var bbhh = 0; bbhh< 8; bbhh++)
                {
